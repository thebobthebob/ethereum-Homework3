...
router.get('/allBalance', async function (req, res, next) {
  ...
  let ethBalance = await web3.eth.getBalance(req.query.account)
  //checkBankBalance就是NCCU Coin餘額，命名沒命好，懶得重部署
  let bankBalance = await bank.methods.checkEtherBalance().call({ from: req.query.account })
  // checkBankBalance就是eth餘額
  let coinBalance = await bank.methods.checkBankBalance().call({ from: req.query.account })
  res.send({
    ethBalance: ethBalance,
...
router.post('/transfer', function (req, res, next) {
  let bank = new web3.eth.Contract(contract.abi);
  bank.options.address = req.body.address;
  bank.methods.transferEther(req.body.to, req.body.value).send({
    from: req.body.account,
    gas: 3400000
  })
    .on('receipt', function (receipt) {
...
//Advanced transfer ether
router.post('/Advancedtransfer', function (req, res, next) {
  let bank = new web3.eth.Contract(contract.abi);
  bank.options.address = req.body.address;
  bank.methods.transferEther(req.body.to, req.body.value).send({
    from: req.body.account,
...
router.get('/owner', async function (req, res, next) {
  ...
  let bank = new web3.eth.Contract(contract.abi);
  bank.options.address = req.query.address;
  let owner = await bank.methods.checkOwner().call({ from: req.query.account });
  res.send({
    owner: owner
  })
});
...
router.post('/mintCoin', function (req, res, next) {
  ...
  let bank = new web3.eth.Contract(contract.abi);
  bank.options.address = req.body.address;
  bank.methods.mint(req.body.MintNumber).send({
    from: req.body.account,
    gas: 3400000
  })
    .on('receipt', function (receipt) {
...
router.post('/buyCoin', function (req, res, next) {
  ...
  let bank = new web3.eth.Contract(contract.abi);
  bank.options.address = req.body.address;
  bank.methods.buy(req.body.BuyNumber).send({
    from: req.body.account,
    gas: 3400000
  })
    .on('receipt', function (receipt) {
...
router.post('/transferOwner', function (req, res, next) {
  ...
  let bank = new web3.eth.Contract(contract.abi);
  bank.options.address = req.body.address;
  bank.methods.transferOwner(req.body.NewOwnerAddress).send({
    from: req.body.account,
    gas: 3400000
  })
    .on('receipt', function (receipt) {
